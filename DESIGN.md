...
A self-contained clone of the Rainette (Jarvis) music player, extracted so it runs
on its own as a native desktop app launched by `Rainette Music.exe`.

## What it is

The full Jarvis music experience with feature parity:
...
music_bridge.py           command handlers (ported from jarvis, overlay removed)
...
- `music_shell.js` — the standalone replacement for jarvis's `rainette_home.js`:
...
- `rainette_music.js` — copied from jarvis; only the two import paths are retargeted
- `rainette_music_player.js` — copied from jarvis; the docked bubble engine (browser fallback)
...
- `rainette_tokens.css`, `rainette_pages.css` — copied verbatim from jarvis
...
### Data flow (unchanged from jarvis)
...
The WebSocket message contract is identical to jarvis, so the copied frontend
...
## Differences from the jarvis version
...
- State is a small music-only SQLite DB (`music.db`), independent of `jarvis_state.db`.
