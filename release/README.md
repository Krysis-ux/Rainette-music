...
`windows-release.json`, verified inside the app against the public key
committed in `release_identity.py`.

1. `python release/new_signing_key.py`
2. Commit the printed **public** key into `release_identity.py`
...
3. CI runs tests, builds the Windows installer (`-Phase Release`) and the
   Android APK, signs `windows-release.json` in an isolated job, and publishes
...
Windows release build without CI (then sign the manifest yourself):
    ...
    python release\sign_manifest.py release\out\windows-release.json
